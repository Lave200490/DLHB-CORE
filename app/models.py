...
class LoanStatus(str, PyEnum):
    ACTIVO = "ACTIVO"
    INACTIVO = "INACTIVO"      # legacy — mantenido para backward compatibility
    CANCELADO = "CANCELADO"    # motor financiero — estado final de un crédito


class Loan(Base):
    """Entidad Préstamo / Crédito."""
...
class InterestRecord(Base):
    """Registro de interés generado — append-only, nunca se modifica ni elimina."""
...
class MoraRecord(Base):
    """Registro de mora acordada manualmente — append-only, nunca se modifica ni elimina."""
...
class Payment(Base):
    """Registro de pago — inmutable tras su creación."""
    ...
    # Columnas legacy (mantenidas para backward compatibility con datos históricos)
    principal_payment = Column(Float, nullable=True, default=0.0)
    interest_payment  = Column(Float, nullable=True, default=0.0)
    ...
    # Snapshot de estado anterior/posterior (D11 — auditoría inline)
    ...
    # Metadatos
    observacion = Column(String(length=500), nullable=True)
    asesor      = Column(String(length=50), nullable=True, default="")
